...
<div class="box">
	...
	<div id="article" class="box_left">
		...
		<div id="content"><?php echo htmlspecialchars_decode($article['article_body']); ?><p>本文最终地址：{$GLOBALS['host_url']}/article/{$article['article_id']}.html</p><p>本文标题：{$article['article_title']}</p><p>本文标签：{volist name="$article['article_tags_array']" id="vo"}<a href="{$GLOBALS['host_url']}/tags/{$vo}.html" target="_blank">{$vo}</a>  {/volist}</p></div>
	</div>
	<div class="box_right">
		<div class="box_title">
			<h3>推荐文章</h3>
